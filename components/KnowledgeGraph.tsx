...
const getNodeColor = (type: NodeType) => {
  switch (type) {
    case 'symptom': return 'bg-orange-100 dark:bg-orange-900/50 text-orange-600 dark:text-orange-400 border-orange-200 dark:border-orange-800';
    case 'disease': return 'bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400 border-red-200 dark:border-red-800';
    case 'medicine': return 'bg-green-100 dark:bg-green-900/50 text-green-600 dark:text-green-400 border-green-200 dark:border-green-800';
    case 'location': return 'bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-800';
...
export const KnowledgeGraph: React.FC<KnowledgeGraphProps> = ({ data, isLoading, onTrain }) => {
  ...
  if (isLoading) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center space-y-6 bg-[#F5F9F7]/90 dark:bg-slate-900/90 backdrop-blur-sm p-8 text-center animate-fade-in relative z-50">
        <Loader2 className="h-12 w-12 text-[#1B4D3E] dark:text-emerald-400 animate-spin" />
        <div className="space-y-2">
          <h3 className="text-xl font-bold text-[#1B4D3E] dark:text-emerald-400 font-kanit">กำลังวิเคราะห์ (Analyzing)</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 font-kanit max-w-[240px] mx-auto leading-relaxed">
             กำลังเชื่อมโยง node...
          </p>
        </div>
      </div>
  ...
  if (data.nodes.length === 0) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center p-8 text-center bg-[#F5F9F7] dark:bg-slate-900 z-40 transition-colors duration-300">
        <div className="mb-6 relative">
           <div className="absolute -inset-4 rounded-full bg-green-50 dark:bg-emerald-900/20 opacity-50 blur-lg"></div>
           <div className="relative flex h-20 w-20 items-center justify-center rounded-2xl bg-white dark:bg-slate-800 shadow-md border border-slate-100 dark:border-slate-700 transform rotate-3">
              <Share2 className="h-10 w-10 text-slate-300 dark:text-slate-600" />
           </div>
        </div>
        <h3 className="text-lg font-bold text-[#1B4D3E] dark:text-emerald-400 font-kanit">ยังไม่มีข้อมูล (No Data)</h3>
  ...
  // --- Graph Visualization ---
  return (
    <div ref={containerRef} className="relative h-full w-full overflow-hidden bg-[#F5F9F7] dark:bg-slate-900 cursor-move z-10 touch-none transition-colors duration-300">
      {/* Background Grid */}
      <div className="absolute inset-0 opacity-[0.05] dark:opacity-[0.1] text-[#1B4D3E] dark:text-slate-500" 
           style={{ 
          ...
          return (
            ...
            >
              <div className="flex h-full w-full items-center justify-center">
                 <div className="relative z-10 px-2.5 py-0.5 bg-white dark:bg-slate-800 rounded-full border border-slate-200 dark:border-slate-700 shadow-md transition-all">
                    <span className="block text-[9px] font-bold text-slate-600 dark:text-slate-300 font-kanit uppercase tracking-wider text-center whitespace-nowrap">
                      {edge.relation}
                    </span>
                 </div>
              </div>
      ...
      {nodes.map((node) => (
        ...
        >
          ...
          <div className={`
             ...
             ${getNodeColor(node.type)} bg-white dark:bg-slate-800 hover:scale-105
          `}>
             <NodeIcon type={node.type} className="h-6 w-6" />
             {/* Type Badge */}
             <div className="absolute -bottom-2.5 rounded-full bg-slate-800 dark:bg-slate-700 px-2 py-0.5 text-[8px] font-bold text-white uppercase tracking-wider border-2 border-white dark:border-slate-800 shadow-sm">
               {node.type}
             </div>
          </div>
          ...
          <div className="mt-3 flex flex-col items-center">
             <span className="rounded-xl bg-white/95 dark:bg-slate-800/95 px-3 py-1.5 text-xs font-bold text-slate-800 dark:text-slate-200 shadow-md backdrop-blur-sm font-kanit whitespace-nowrap border border-slate-100 dark:border-slate-700 text-center max-w-[140px] leading-tight">
               {node.label}
             </span>
          </div>
        </div>
      ...
      {data.nodes.length > 0 && onTrain && !isLoading && (
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-50 animate-fade-in-up">
            <button 
              ...
              className="group flex items-center gap-3 rounded-full bg-[#1B4D3E] dark:bg-emerald-600 px-8 py-4 text-white shadow-2xl shadow-green-500/20 hover:scale-105 active:scale-95 transition-all ring-4 ring-white/30 dark:ring-emerald-900/30 hover:ring-white/50 hover:bg-[#143d30] dark:hover:bg-emerald-700"
            >
              ...
              <div className="flex flex-col items-start">
                 ...
                 <span className="text-[10px] font-medium text-green-50 leading-none mt-1">Add {data.nodes.length} nodes to brain</span>
              </div>
            </button>
        </div>
      )}
